  ...
  module V1
    class Metrics < Roda
      ...
      route do |r|
        ...
        #  1          3            4           5                    6
        # sum:system.cpu.idle{db.type:sql}.per_second() by {timestamp,component}
        ...
        cte = MetricDatum
          .select("metric_data.timestamp")
          .select("metric_data.tags")
          .select("SUM(metric_data.value) AS sum")
          .select("COUNT(metric_data.*) AS count")
          .joins(:metric)
          .where(timestamp: @time_range)
          .where("metric_data.value != 'NaN'")
          .group("metric_data.timestamp")
          .group("metric_data.tags")
        ...
        if tags.present?
          keys = (tags.map(&:first) + groups).uniq
          keys.each do |key|
            cte = cte.where("metric_data.tags ? :key", key: key)
          end
        end
